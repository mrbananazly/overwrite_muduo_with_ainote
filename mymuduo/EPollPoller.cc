...
EPollPoller::EPollPoller(EventLoop *loop)
    : Poller(loop), epollfd_(::epoll_create1(EPOLL_CLOEXEC)), events_(kInitEventListSize) // vector<epoll_event>(16)
{
    if (epollfd_ < 0)
    {
...
void EPollPoller::update(int operation, Channel *channel)
{
    ...
    // 使用 Linux 中的 epoll 实现对一个文件描述符 fd 进行 add、modify 或 delete 操作
