...
void EventLoopThreadPool::start(const ThreadInitCallback &cb)
{
    ...
    for (int i = 0; i < numThreads_; i++) // 访问线程池中的线程
    {
        char buf[name_.size() + 32];                         // 存储线程的名称
        snprintf(buf, sizeof buf, "%s%d", name_.c_str(), i); // 将线程池名称与线程编号拼接成一个字符串存储在buf中
        EventLoopThread *t = new EventLoopThread(cb, buf);
        threads_.push_back(std::unique_ptr<EventLoopThread>(t));
        loops_.push_back(t->startLoop()); // 底层创建线程
    }
